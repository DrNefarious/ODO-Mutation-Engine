...
#include<regex>			//To check for punctuation
...
int CommandHandler(vector<string>& commands, string& command) {//The purpose of this function is to split the user's input into chunks
	...
	for (unsigned short int i = 0; i < command.length(); i++) {
...
void decompressor(string& inputString, vector<unsigned char>& outputVector) {
	...
	for (unsigned short int i = 0; i < inputString.length(); i++) {
		if (inputString[i] == 'x') {
			...
			for (unsigned short int j = 0; j < count; j++) {
...
int assembly(string payloadFile, string mutatorFile, string targetArch, string fileName) {
	...
	for (unsigned short int i = 0; i < fileName.length(); i++) {
	...
	for (unsigned short int i = 0; i < tempName.length(); i++) {
	...
	unsigned short int fileSize = tempBinary.size() / 2;  //Dividing by 2 since the tempbinary deals with half bytes.
	unsigned short int tempAddress[4];
	for (short int i = 3; i > -1; i--) {
		unsigned short int power = pow(16, i);
		cout << i << " - " << power << endl;
		unsigned short int count = floor(fileSize / power);
		tempAddress[i] = count;
	...
	for (unsigned short int i = 0; i < tempBinary.size(); i++) {
		int sum = 0;
		if (tempBinary[i] <= '9') {
...
int main(int argc, char* argv[]) {
	...
	do {//Begin Main Loop, loops until command == "quit"
		if (!clv) {
			...
			transform(command.begin(), command.end(), command.begin(), ::tolower);	//Making command lowercase for matches
		...
		if (commands[0].compare(acceptedCommands[0]) == 0) {
			...
			if (commands.size() == 9) {//Quickly checks to see if there are enough arguments given.
				...
				unsigned short int parCheck[4] = { 0,0,0,0 };
